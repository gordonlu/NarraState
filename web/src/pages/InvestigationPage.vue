...
<template>
  <div class="investigation-shell">
    ...
    <main v-if="store.session && store.activeCase" class="investigation-grid" :data-mobile-tab="mobileTab">
      <aside class="people-panel workspace-people">
        <section>
          ...
          <button
            ...
            type="button"
            @click="store.activeCharacterId = character.id; mobileTab = 'dialogue'"
          >
      ...
      <section class="dialogue-panel workspace-dialogue">
        ...
        <div ref="transcript" class="transcript" aria-live="polite">
          ...
          <article v-for="entry in store.session.conversation" :key="`${entry.turn_id}-${entrySpeaker(entry)}`" class="transcript-turn" :class="{ player: speakerId(entry.speaker) === 'Player' }">
        ...
        <div class="composer-region">
          <div class="attachment-summary"><span><AppIcon name="paperclip" :size="18" />已附加 {{ store.selectedEvidence.length }} 条线索</span><button type="button" @click="mobileTab = 'research'; researchTab = 'evidence'">选择线索</button></div>
          <div v-if="store.selectedEvidence.length" class="attachment-list">
            <div v-for="item in store.selectedEvidence" :key="item.id" class="attachment-row"><AppIcon name="document" /><span>{{ item.title }}</span><button type="button" :aria-label="`移除 ${item.title}`" @click="store.toggleEvidence(item.id)"><AppIcon name="close" :size="18" /></button></div>
          </div>
      ...
      <aside class="research-panel workspace-research">
        ...
        <div v-if="researchTab === 'evidence'" class="research-content evidence-content">
          ...
          <label v-for="item in store.session.discovered_evidence" :key="item.id" class="evidence-row" :class="{ attached: store.attachedEvidenceIds.includes(item.id) }">
            <input type="checkbox" :checked="store.attachedEvidenceIds.includes(item.id)" @change="store.toggleEvidence(item.id)" />
            <AppIcon name="document" :size="23" />
