...
案件生成采用“蓝图 + 共享世界 + 逐角色并发 + 独立真相变体”的分段输出，通常调用次数为 `2 + 角色数量 + 真相变体数量`；角色与变体分别并发，单个失败变体可以独立纠错。每个分段的结构化输出预算默认为 65536 token，可通过 `NARRASTATE_GENERATION_MAX_TOKENS` 在 4096–65536 之间调低。该设置只影响案件生成，不会让普通角色对话变得冗长；模型或兼容服务仍可能应用自身更低的输出上限。
